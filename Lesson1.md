...
Turn off SELinux (required)
Edit /etc/selinux/config (change PERMISSIVE to disable)
Remove swap mount (required)
Either run swapoff or by removing / commenting out the mount in /etc/fstab (and rebooting)
Install EPEL and open-vm-tools package (required in VMs)
        ```yum install epel-release -y
          yum install open-vm-tools -y```
				□ Password-less SSH setup from master node to all others
I'll also setup Clusterssh
...
```
			...
			3. Test Docker
				...
				2) hostname
				3) Verify that the hostname is different than the host you're on
				4) exit
	Kubernetes tools
			1. Now we need to install all the Kubernetes tools including:
				1) Kubeadm - Installation utility
				2) Kubelet - The heavy-lifting component of the Kubernetes runtime
				3) Kubectl - The CLI tool
	...
	Initialize your Kubernetes cluster
			1. On the master only:
				...
				3) Test
					a) kubectl get nodes
					b) You should see your 1 master with a status of ready (may take a minute or two)
	...
	Network Overlay
		Serves to unite Kubernetes nodes in an overlay allowing containers on each host to talk not just to themselves, but to each other.
			1. Install Calico (https://docs.projectcalico.org/getting-started/kubernetes/quickstart)
			2. kubectl create -f https://docs.projectcalico.org/manifests/tigera-operator.yaml
			3. kubectl create -f https://docs.projectcalico.org/manifests/custom-resources.yaml
			4. Check to make sure Calico comes online (3 services all have 1/1 containers running)
				1) watch kubectl get pods -n calico-system
	Join the workers to the cluster
			1. Run on EACH of the remaining nodes:
				1) The "join" string you wrote down after the kubeadm init
			2. On the master check that you have 1 master and 3 workers (Role=<none>)
				1) "kubectl get nodes" should look like the following:
	...
	Network Loadbalancer
			1. MetalLB
				1) Install Metallb: https://metallb.universe.tf/installation/
					a) kubectl edit configmap -n kube-system kube-proxy
					b) Change the line about strictARP to true
					c) Exit with :wq
					d) Run the below:
						kubectl apply -f https://raw.githubusercontent.com/metallb/metallb/v0.9.3/manifests/namespace.yaml
						kubectl apply -f https://raw.githubusercontent.com/metallb/metallb/v0.9.3/manifests/metallb.yaml
						kubectl create secret generic -n metallb-system memberlist --from-literal=secretkey="$(openssl rand -base64 128)"
					e) Now we need to create a config that lists what IPs we want to use for our LoadBalancer
						i) vi metallb.yaml
						ii) Paste in the below and change the IP range to the one you set aside according to the pre-requirements:
							apiVersion: v1
							kind: ConfigMap
							metadata:
						...
						iii) Add the configuration we created above into kubernetes:
